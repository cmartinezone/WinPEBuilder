...
:: Where to put Windows PE tree and ISO
set winpe_root=%~dp0WinPE-Root
 
:: ADK installation path. ADK 10 can be found here: https://docs.microsoft.com/en-us/windows-hardware/get-started/adk-install
set adk_path=%programfiles(x86)%\Windows Kits\10\Assessment and Deployment Kit
 
:: Drivers tree path
set drivers_path=%~dp0Add-Drivers

:: User scripts location to include in new WinPE image. Scripts tree should include the modified startnet.cmd in which you can add your stuff
set scripts_path=%~dp0Add-Scripts

:: Directory path for finall iso generated
...
:: Adding some useful packages. Packages description and dependencies for WinPE 10 can be found here: http://technet.microsoft.com/en-us/library/hh824926.aspx
...
:: Bitlocker startup support packages
...
:: Remove  winpe background permisions to be alegible for replacement.
...
:: Copying script to the WinPE root directory
...
:: Unmounting and commit changes
Dism /Unmount-Wim /MountDir:%winpe_root%\mount\ /Commit

:: Creating ISO image from WinPE 
